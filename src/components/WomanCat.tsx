...
import { fetchCatWoman } from "../api/fetchapi";

export const WomanCat = () => {
    interface womancat {
        id: number;
        catName: string;
        mainImage: string,
    ...
    const [womanCats, setWomanCats] = useState<womancat[]>([]);

    useEffect(() => {
        fetchCatWoman().then(setWomanCats);
    }, []);
    ...
    return (
        <div className="_6BoxOwl carousel_modular2 wide_carousel_controls position-relative mt-md-4 mt-2 d-flex align-items-center">         
                ...
                >
                    {womanCats.map((womanCat, index) => (
                        <SwiperSlide>
                            <div className="_6BoxOwl--box  ml-20 d-flex flex-column border justify-center justify-center items-center">
                                <img src={womanCat.mainImage} className="--radiuseTop img-fluid" />
                                <span className="style-20 --black my-1 client-cell__body card-title__ellipsis">{womanCat.catName}</span>
                            </div>
                        </SwiperSlide>
                    ))}
